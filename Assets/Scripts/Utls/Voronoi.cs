...
/*
* 
* C# move form double[] to Vector2 and some other unity improvements by Chase Wenner
* 
* Permission to use, copy, modify, and distribute this software for any
* purpose without fee is hereby granted, provided that this entire notice
* is included in all copies of any software which is or includes a copy
       ...
       public void reset()
        {
            ...
            sites = null;
            ELhash = null;
            if(allEdges != null) allEdges.Clear();
            allEdges = null;
            nvertices = nedges = nsites = 0;
        }
       /**
        * 
...
// تستدعى هذه العملية لإنشاء مخطط فورونوي
        public List<GraphEdge> generateVoronoi(List<Vector2> points, Rect bounds) //to make things easier
        {
            double[] ptx = new double[points.Count], pty = new double[points.Count];
            for (int i = 0; i < points.Count; i++) { ptx[i] = points[i].x; pty[i] = points[i].y; }

            return generateVoronoi(ptx, pty, bounds.xMin, bounds.xMax-1, bounds.xMin, bounds.yMax-1);
        }
